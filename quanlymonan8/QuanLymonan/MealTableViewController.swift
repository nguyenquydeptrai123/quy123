...
class MealTableViewController: UITableViewController {
    ...
    */
    ...
    override func tableView(_ tableView: UITableView, commit editingStyle: UITableViewCell.EditingStyle, forRowAt indexPath: IndexPath) {
        if editingStyle == .delete {
            ...
            if dao.open(){
                dao.delete(meal: mealList[indexPath.row])
            }
    ...
    */
    ...
    private func loadMeal(){
        if dao.open(){
            if !MealTableViewController.isCreateTable{
                MealTableViewController.isCreateTable = dao.createTable()
            }
            dao.readMeal(meals: &mealList)
        }
        if mealList.count == 0{
        ...
        case .updateMeal:
            ...
            if let sourceController = sender.source as? ViewController, let updateMeal = sourceController.meal {
                if let selectedCellIndexPath = tableView.indexPathForSelectedRow{
                    if dao.open(){
                        dao.update(olMeal: mealList[selectedCellIndexPath.row], newMeal: updateMeal)
                    }
                }
